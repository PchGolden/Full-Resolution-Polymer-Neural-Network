...
def main():
    ...
    raw = rows(root / 'data/raw/md_final1640_v2/final_1640dataset.csv')
    folds = rows(root / 'data/splits/md_final1640_v2/with_fold.csv')
    assert len(raw) == len(folds) == 1640
    assert {int(r['fold']) for r in folds} == set(range(5))
    mapping = rows(root / 'data/lammps/dataset_record_mapping.csv')
    assert {int(r['dataset_row']) for r in mapping} == set(range(1640))
    for row in mapping:
        path = root / 'data/lammps' / row['lammps_file']
        assert path.is_file(), path
        if args.lammps_hashes:
            assert sha256(path) == row['sha256'], path
    oof = list((root / 'results/oof_predictions/md_final1640_v2').glob('oof_*.csv'))
    assert len(oof) == 28, len(oof)
    for path in oof:
        assert len(rows(path)) == 1640, path
    assert len(rows(root / 'results/diagnostics/md/structure_variation_pairs.csv')) == 1417
    assert len(rows(root / 'results/diagnostics/md/chemistry_control_pairs.csv')) == 29027
    manifest = rows(root / 'docs/checkpoint_manifest.csv')
