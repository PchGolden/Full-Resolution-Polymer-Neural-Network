...
Example:
  python -m reproduce.evaluate.bcdb_export \
    --checkpoint reproduce/outputs/training/results_xxx/.../checkpoints/checkpoint.pt \
    --pkl_path datasets/processed/BCDB_chain_withvolume.pkl \
    --fold 0 \
    --output_npz reproduce/outputs/analysis/extra_ablation_probs_20260522/model/fold_0.npz
"""
...
from frpn.bcdb.data import build_dataloader
from frpn.bcdb.models.model import MultiMolModel
...
def main(argv=None) -> int:
    ...
    p.add_argument("--pkl_path", required=True, help="Path to datasets/processed/*.pkl (with folds)")
    p.add_argument("--fp_csv", default=str(Path(__file__).resolve().parents[2] / "datasets/raw/bcdb/bcdb.csv"), help="Released monomer CSV for fingerprint table construction")
