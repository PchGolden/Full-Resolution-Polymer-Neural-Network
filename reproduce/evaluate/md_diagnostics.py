...
def _build_topology_pairs(meta: pd.DataFrame, comp_vec: np.ndarray, threshold: float = 0.95) -> pd.DataFrame:
    """Structure-control pairs: same temperature and exact length, chemistry overlap,
    and different topology, ordering, or both (final manuscript definition).
    """
    rows = []
    for (temperature, chain_len), group in meta.groupby(["glob_feat0", "chain_len"], dropna=False):
        ...
        if len(indices) < 2:
            continue
        overlaps = _pairwise_overlaps(comp_vec, indices, indices)
        left, right = np.triu_indices(len(indices), k=1)
        keep = overlaps[left, right] >= float(np.float32(threshold))
        for a, b in zip(left[keep], right[keep]):
            i, j = int(indices[a]), int(indices[b])
            if str(meta.at[i, "topology"]) == str(meta.at[j, "topology"]) and str(meta.at[i, "mix_mode"]) == str(meta.at[j, "mix_mode"]):
                continue
            rows.append(dict(match_type="chemistry_matched_structure_variation", glob_feat0=float(temperature), chain_len=int(chain_len), row_i=i, row_j=j, overlap=float(overlaps[a,b]), topology_i=str(meta.at[i,"topology"]), topology_j=str(meta.at[j,"topology"]), ordering_i=str(meta.at[i,"mix_mode"]), ordering_j=str(meta.at[j,"mix_mode"])))
...
#SBATCH --mem={mem}
#SBATCH --time={time}
#SBATCH --output={shlex.quote(str((out_dir / "slurm_tsne_%A_%a.out").resolve()))}
#SBATCH --error={shlex.quote(str((out_dir / "slurm_tsne_%A_%a.err").resolve()))}
...
python -m reproduce.evaluate.md_tsne \
  --raw_csv {shlex.quote(str(raw_csv.resolve()))} \
  --embeds_dir {shlex.quote(str(embeds_dir.resolve()))} \
  --preds_dir {shlex.quote(str(preds_dir.resolve()))} \
  --model \"$MODEL\" \
  --label \"$LABEL\" \
  --out_dir {shlex.quote(str(tsne_out_dir.resolve()))} \
  --value y_pred_raw \
  --cmap blue_navy \
...
def main() -> int:
    ...
    _plot_heatmap(
        ...
        title="Structure control: corr(Δŷ_z, Δy_z)",
