...
def _predict_one_fold(
    ...
    *,
    ...
    from frpn.bcdb import train as bcdb_main  # type: ignore
    from frpn.bcdb.models.model import MultiMolModel  # type: ignore
    from frpn.bcdb.data import build_dataloader  # type: ignore

    ckpt = torch.load(str(ckpt_path), map_location="cpu")
    args_dict = ckpt.get("args", {})

    # Rebuild args object using main.parse_args(), then override with ckpt args.
    # This is safer than constructing a raw Namespace since -m frpn.bcdb.train expects many fields.
    argv = [
        "prog",
        "--dataset_name",
        "_eval",
...
def main() -> int:
    ...
    ap.add_argument("--results_root", default="reproduce/outputs/training/results_homopolymer_mdstyle/HOMO_372")
    ap.add_argument("--pkl_dir", default="datasets/processed/HOMO_372/homopolymer/main")
    ap.add_argument("--out_dir", default="reproduce/outputs/analysis/homopolymer_oof_scatter")
    ap.add_argument("--cache-dir", type=Path, help="Read retained per-model target caches from this directory")
    ...
    cache_dir = args.cache_dir or (out_dir / "cache_oof_npz")
    ...
    for label_key, label_raw in LABEL_KEYS:
        ...
        for spec in specs:
            ...
            use_cache = bool(args.cache or args.cache_dir) and not bool(args.no_cache)
            if use_cache and cache_path.exists():
                ...
                for fold in range(int(args.folds)):
                    ...
                    packaged = results_root / spec.model_tag / label_key / f"{spec.model_tag}__{label_key}__fold{fold}.pt"
                    ckpt = packaged if packaged.is_file() else _latest_ckpt_for_fold(label_root, fold)
                    if ckpt is None:
