...
def _run_export(export_module: str, ckpt: str, pkl: str, fold: int, out_npz: str, device: str, batch_size: int) -> None:
    cmd = [
        sys.executable,
        "-m",
        export_module,
...
def main(argv=None) -> int:
    ...
    p.add_argument("--pkl_path", default="datasets/processed/BCDB_chain_withvolume.pkl")
    p.add_argument("--output_root", default="reproduce/outputs/analysis/extra_ablation_metrics_20260523")
    p.add_argument("--device", default="cuda", choices=["cuda", "cpu"])
    p.add_argument("--batch_size", type=int, default=64)

    p.add_argument(
        "--stage2_fp_root",
        default="reproduce/outputs/training/results_stage2_fp_morgan2048_withvol/BCDB",
        help="Root dir containing stage2 fp runs (will pick latest checkpoint.pt per fold).",
    )
    p.add_argument(
        "--frpn_ptfreeze_root",
        default="reproduce/outputs/training/results_frpn_ptfreeze_stage1_withvol/BCDB",
        help="Root dir containing frpn ptfreeze runs (will pick latest checkpoint.pt per fold).",
    )
    p.add_argument(
        "--frpn_full_ckpt_pattern",
        default=(
            "reproduce/outputs/training/BCDB/task_chain_eq_0_freeze_0/"
    ...
    p.add_argument(
        ...
        default="reproduce/outputs/training/results_frpn_fullchain_pt_withvol_100ep/BCDB",
    ...
    export_module = "reproduce.evaluate.bcdb_export"
