...
class PackageImportTests(unittest.TestCase):
    def test_both_model_families_coexist(self):
        from frpn.bcdb import data as linear_data, train as linear_train
        from frpn.bcdb.models.model import MultiMolModel as LinearModel
    ...
    def test_all_modules_import_without_running_a_command(self):
        ...
        packages = (
            (root / "src" / "frpn", root / "src"),
            (root / "reproduce" / "evaluate", root),
            (root / "reproduce" / "tools", root),
        )
        for directory, import_root in packages:
            for path in sorted(directory.rglob("*.py")):
                if path.name == "__init__.py":
                    continue
                name = ".".join(path.relative_to(import_root).with_suffix("").parts)
