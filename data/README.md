...
- `raw/md_final1640_v2/final_1640dataset.csv`: 1,640-record MD benchmark in the released final row order.
...
For LAMMPS, `dataset_record_mapping.csv` uses one-based record numbers and zero-based CSV row indices. File paths are relative to `data/lammps/`; SHA-256 hashes identify the exact simulation inputs. No released MD record is designated as a failed simulation.
