...
`docs/checkpoint_manifest.csv` maps each extracted checkpoint to its dataset, model, target and fold. `release_path` is relative to the extracted `checkpoints/` directory. The archived MD lookup layout is `<model>/<label>/<model>__<label>__foldN.pt`; the exporter recognizes this layout directly.

The checkpoint manifest includes the main BCDB models and capacity controls (including FRPN-tiny), the three homopolymer models, four MD models across seven targets, and the initialization checkpoints. Additional volume/anchor ablation results are retained in `results/ablations/bcdb_si_s11/`.
